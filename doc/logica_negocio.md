...
### `sesiones_reto`
Solo para retos tipo `RECORRIDO`: guarda el marco (`inicio`/`fin`/`estado`) de una sesión de tracking en vivo. El tracking GPS punto-a-punto en sí vive en Redis mientras la sesión está en curso (lista `session:points:{sesion_id}`, un `RPUSH` por punto, TTL de seguridad de 24h) — esta tabla es el registro persistente, no el stream. Al cerrar la sesión (`PATCH .../finish`), la aplicación lee todos los puntos, calcula la distancia total con la fórmula de Haversine y la key de Redis se borra — no queda nada ahí después de finalizar. Esa distancia (en metros) se acredita automáticamente como progreso del reto, con el mismo camino que usa `POST /challenges/{id}/progress` (puntos, racha, hitos, canje si aplica): para retos `RECORRIDO`, `cantidad_requerida` se interpreta como metros a recorrer.
